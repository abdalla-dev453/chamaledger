...
export const api = {
  ...
  groups: {
    ...
    update: (groupId, data) =>
      request(`/api/v1/groups/${groupId}`, { method: "PATCH", body: data }),
    listMembers: (groupId) => request(`/api/v1/groups/${groupId}/members`),
    updateMember: (groupId, memberId, data) =>
      request(`/api/v1/groups/${groupId}/members/${memberId}`, {
        method: "PATCH",
        body: data,
      }),
  },
  ...
  cycles: {
    ...
    create: (groupId, data) =>
      request(`/api/v1/cycles/${groupId}`, { method: "POST", body: data }),
    close: (groupId, cycleId) =>
      request(`/api/v1/cycles/${groupId}/${cycleId}/close`, {
        method: "PATCH",
      }),
    summary: (groupId, cycleId) =>
      request(`/api/v1/cycles/${groupId}/${cycleId}/summary`),
  },

  contributions: {
    list: (groupId, cycleId) =>
      request(`/api/v1/contributions/${groupId}/cycles/${cycleId}`),
    ...
    updateStatus: (groupId, contributionId, status) =>
      request(`/api/v1/contributions/${groupId}/${contributionId}`, {
        method: "PATCH",
        body: { status },
      }),
  },
  ...
  loans: {
    ...
    issue: (groupId, data) =>
      request(`/api/v1/loans/${groupId}`, { method: "POST", body: data }),
    get: (groupId, loanId) => request(`/api/v1/loans/${groupId}/${loanId}`),
    repay: (groupId, loanId, data) =>
      request(`/api/v1/loans/${groupId}/${loanId}/repayments`, {
        method: "POST",
        body: data,
      }),
  },
  ...
  mpesa: {
    stkPush: (data) =>
      request("/api/v1/mpesa/stkpush", { method: "POST", body: data }),
  },
};
